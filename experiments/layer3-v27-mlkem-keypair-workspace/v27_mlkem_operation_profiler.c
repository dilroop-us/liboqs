 ...
 */
static uint64_t v27_rng_state = 0x123456789abcdef0ull;

static uint64_t v27_splitmix64(void) {
    uint64_t z = (v27_rng_state += 0x9e3779b97f4a7c15ull);
    z = (z ^ (z >> 30)) * 0xbf58476d1ce4e5b9ull;
    z = (z ^ (z >> 27)) * 0x94d049bb133111ebull;
    return z ^ (z >> 31);
}

static void v27_randombytes(uint8_t *out, size_t out_len) {
    size_t i = 0;

    while (i < out_len) {
        uint64_t r = v27_splitmix64();
...
int main(int argc, char **argv) {
    ...
    OQS_randombytes_custom_algorithm(v27_randombytes);
