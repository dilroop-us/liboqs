...
if len(sys.argv) != 4:
    print("usage: v23b_stack_usage_analyzer.py <build-dir> <label> <results-dir>")
    sys.exit(2)
...
csv_path = RESULTS / f"v23b_{LABEL}_stack_usage.csv"
top_path = RESULTS / f"v23b_{LABEL}_top_stack_usage.md"
x86_path = RESULTS / f"v23b_{LABEL}_x86_64_top_stack_usage.md"
targets_path = RESULTS / f"v23b_{LABEL}_targets.md"
...
def write_top(path, title, filtered_rows):
    with path.open("w") as f:
        f.write(f"# v23b top ML-DSA stack-usage functions: {title}\n\n")
...
with targets_path.open("w") as f:
    f.write("# v23b target ML-DSA stack frames\n\n")
