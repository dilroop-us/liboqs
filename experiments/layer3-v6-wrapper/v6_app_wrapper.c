...
static int run_kem_benchmark(size_t iterations) {
...
static int run_sig_benchmark(size_t iterations) {
    ...
    const uint8_t message[] = "Layer 3 v6 custom wrapper benchmark message";
...
int main(int argc, char **argv) {
    ...
    printf("Layer 3 v6 custom liboqs wrapper benchmark\n");
    printf("Build target: minimal no-OpenSSL liboqs\n\n");

    if (run_kem_benchmark(iterations) != EXIT_SUCCESS) {
        return EXIT_FAILURE;
    }

    if (run_sig_benchmark(iterations) != EXIT_SUCCESS) {
        return EXIT_FAILURE;
    }
