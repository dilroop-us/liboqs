 ...
 */
static uint64_t v29_rng_state = 0x123456789abcdef0ull;

static uint64_t v29_splitmix64(void) {
    uint64_t z = (v29_rng_state += 0x9e3779b97f4a7c15ull);
    z = (z ^ (z >> 30)) * 0xbf58476d1ce4e5b9ull;
    z = (z ^ (z >> 27)) * 0x94d049bb133111ebull;
    return z ^ (z >> 31);
}

static void v29_randombytes(uint8_t *out, size_t out_len) {
    size_t i = 0;

    while (i < out_len) {
        uint64_t r = v29_splitmix64();
...
/* v29 compact lifecycle workspace symbols */
extern size_t PQCP_MLKEM_NATIVE_MLKEM768_X86_64_v29_lifecycle_workspace_bytes(void) __attribute__((weak));
extern int PQCP_MLKEM_NATIVE_MLKEM768_X86_64_v29_lifecycle_workspace_set(void *workspace, size_t workspace_bytes) __attribute__((weak));

extern size_t PQCP_MLKEM_NATIVE_MLKEM768_C_v29_lifecycle_workspace_bytes(void) __attribute__((weak));
extern int PQCP_MLKEM_NATIVE_MLKEM768_C_v29_lifecycle_workspace_set(void *workspace, size_t workspace_bytes) __attribute__((weak));
...
static size_t get_v26_enc_workspace_bytes(void) {
    size_t x86_bytes = 0;
    size_t ref_bytes = 0;
    ...
    return max_size(x86_bytes, ref_bytes);
}

static size_t get_v27_keypair_workspace_bytes(void) {
    size_t x86_bytes = 0;
    size_t ref_bytes = 0;
...
static size_t get_v28_dec_workspace_bytes(void) {
    size_t x86_bytes = 0;
    size_t ref_bytes = 0;
...
static void setup_v29_lifecycle_workspace(const char *profile, const char *workspace_csv) {
    size_t x86_bytes = 0;
    size_t ref_bytes = 0;
    size_t lifecycle_bytes = 0;
    size_t enc_bytes = 0;
    size_t keypair_bytes = 0;
    size_t dec_bytes = 0;
    size_t separate_total = 0;
    size_t saved_bytes = 0;
    void *workspace = NULL;
    int rc = 0;

    if (PQCP_MLKEM_NATIVE_MLKEM768_X86_64_v29_lifecycle_workspace_bytes != NULL &&
        PQCP_MLKEM_NATIVE_MLKEM768_X86_64_v29_lifecycle_workspace_set != NULL) {
        x86_bytes = PQCP_MLKEM_NATIVE_MLKEM768_X86_64_v29_lifecycle_workspace_bytes();
    }

    if (PQCP_MLKEM_NATIVE_MLKEM768_C_v29_lifecycle_workspace_bytes != NULL &&
        PQCP_MLKEM_NATIVE_MLKEM768_C_v29_lifecycle_workspace_set != NULL) {
        ref_bytes = PQCP_MLKEM_NATIVE_MLKEM768_C_v29_lifecycle_workspace_bytes();
    }

    lifecycle_bytes = max_size(x86_bytes, ref_bytes);

    if (lifecycle_bytes == 0) {
        die("v29 lifecycle workspace symbols not found");
    }

    enc_bytes = get_v26_enc_workspace_bytes();
    keypair_bytes = get_v27_keypair_workspace_bytes();
    dec_bytes = get_v28_dec_workspace_bytes();
    separate_total = enc_bytes + keypair_bytes + dec_bytes;

    if (separate_total > lifecycle_bytes) {
        saved_bytes = separate_total - lifecycle_bytes;
    }

    workspace = xaligned_alloc_64(lifecycle_bytes);

    if (PQCP_MLKEM_NATIVE_MLKEM768_X86_64_v29_lifecycle_workspace_set != NULL) {
        rc = PQCP_MLKEM_NATIVE_MLKEM768_X86_64_v29_lifecycle_workspace_set(workspace, lifecycle_bytes);

        if (rc != 0) {
            fprintf(stderr, "x86_64 v29 lifecycle workspace set failed: %d\n", rc);
            exit(1);
        }
    }

    if (PQCP_MLKEM_NATIVE_MLKEM768_C_v29_lifecycle_workspace_set != NULL) {
        rc = PQCP_MLKEM_NATIVE_MLKEM768_C_v29_lifecycle_workspace_set(workspace, lifecycle_bytes);

        if (rc != 0) {
            fprintf(stderr, "ref v29 lifecycle workspace set failed: %d\n", rc);
            exit(1);
        }
    }

    append_workspace_csv(workspace_csv, profile, "v26_enc_workspace_separate", enc_bytes);
    append_workspace_csv(workspace_csv, profile, "v27_keypair_workspace_separate", keypair_bytes);
    append_workspace_csv(workspace_csv, profile, "v28_dec_workspace_separate", dec_bytes);
    append_workspace_csv(workspace_csv, profile, "v29_separate_total", separate_total);
    append_workspace_csv(workspace_csv, profile, "v29_lifecycle_workspace", lifecycle_bytes);
    append_workspace_csv(workspace_csv, profile, "v29_saved_bytes", saved_bytes);

    printf("v26_enc_workspace_separate=%zu\n", enc_bytes);
    printf("v27_keypair_workspace_separate=%zu\n", keypair_bytes);
    printf("v28_dec_workspace_separate=%zu\n", dec_bytes);
    printf("v29_separate_total=%zu\n", separate_total);
    printf("v29_lifecycle_workspace=%zu\n", lifecycle_bytes);
    printf("v29_saved_bytes=%zu\n", saved_bytes);
}
...
int main(int argc, char **argv) {
    ...
    OQS_randombytes_custom_algorithm(v29_randombytes);
    ...
    setup_v29_lifecycle_workspace(profile, workspace_csv);
