...
def main() -> int:
    if len(sys.argv) != 4:
        print("usage: v17b_stack_usage_analyzer.py <build_dir> <profile> <out_dir>", file=sys.stderr)
        return 2
    ...
    csv_path = out_dir / f"v17b_{profile}_stack_usage.csv"
    top_path = out_dir / f"v17b_{profile}_top_stack_usage.md"
    x86_top_path = out_dir / f"v17b_{profile}_x86_64_top_stack_usage.md"
    ...
    write_md(top_path, f"v17b top ML-DSA stack-usage functions: {profile}", rows)
    write_md(
        x86_top_path,
        f"v17b top ML-DSA x86_64 stack-usage functions: {profile}",
