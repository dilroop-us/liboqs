...
FLAG = "MLK_CONFIG_EXPERIMENTAL_CALLER_ENC_WORKSPACE"
SOURCE_FN = "mlk_indcpa_enc"
...
def insert_v26_block(text: str, prefix: str):
    marker = f"{prefix}_v26_enc_workspace_bytes"
...
size_t {prefix}_v26_enc_workspace_bytes(void) {{
    return sizeof(mlk_v26_enc_workspace);
}}

int {prefix}_v26_enc_workspace_set(void *workspace, size_t workspace_bytes) {{
...
def patch_enc_function(text: str):
    ...
    if "MLK_V26_REQUIRE_ENC_WORKSPACE" in body:
        return text

    alloc_macro = r"(?:MLKEM_ALLOC|MLK_ALLOC|KYBER_ALLOC)"

    alloc_pat = re.compile(
        rf"(?P<indent>[ \t]*){alloc_macro}\s*\(\s*seed\s*,.*?"
        rf"{alloc_macro}\s*\(\s*sp_cache\s*,.*?;\s*\n",
        re.S,
    )
...
{indent}MLK_V26_REQUIRE_ENC_WORKSPACE();

{indent}uint8_t *seed = mlk_v26_tls_enc_workspace->seed;
{indent}mlk_polymat *at = &mlk_v26_tls_enc_workspace->at;
{indent}mlk_polyvec *sp = &mlk_v26_tls_enc_workspace->sp;
{indent}mlk_polyvec *pkpv = &mlk_v26_tls_enc_workspace->pkpv;
{indent}mlk_polyvec *ep = &mlk_v26_tls_enc_workspace->ep;
{indent}mlk_polyvec *b = &mlk_v26_tls_enc_workspace->b;
{indent}mlk_poly *v = &mlk_v26_tls_enc_workspace->v;
{indent}mlk_poly *k = &mlk_v26_tls_enc_workspace->k;
{indent}mlk_poly *epp = &mlk_v26_tls_enc_workspace->epp;
{indent}mlk_polyvec_mulcache *sp_cache = &mlk_v26_tls_enc_workspace->sp_cache;
{indent}#else
...
'''
    ...
    names = ["seed", "at", "sp", "pkpv", "ep", "b", "v", "k", "epp", "sp_cache"]
...
{free_indent}MLK_V26_CLEAN_ENC_WORKSPACE();
{free_indent}#else
{original_free_block}
{free_indent}#endif'''
...
def patch_file(label: str, path: Path, prefix: str):
    ...
    backup = path.with_suffix(path.suffix + ".before_v26")
    ...
    text = insert_v26_block(text, prefix)
    text = patch_enc_function(text)
