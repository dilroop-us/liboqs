...
extern int PQCP_MLDSA_NATIVE_MLDSA44_X86_64_v24_lifecycle_workspace_set(void *workspace, size_t workspace_bytes) __attribute__((weak));

extern size_t PQCP_MLDSA_NATIVE_MLDSA44_C_v24_lifecycle_workspace_bytes(void) __attribute__((weak));
extern int PQCP_MLDSA_NATIVE_MLDSA44_C_v24_lifecycle_workspace_set(void *workspace, size_t workspace_bytes) __attribute__((weak));

static void *v24_x86_workspace;
static void *v24_c_workspace;
...
static void setup_v24_workspace_if_available(void) {
  if (PQCP_MLDSA_NATIVE_MLDSA44_X86_64_v24_lifecycle_workspace_bytes &&
    ...
    size_t bytes = PQCP_MLDSA_NATIVE_MLDSA44_X86_64_v24_lifecycle_workspace_bytes();

    if (v24_x86_workspace == NULL) {
      v24_x86_workspace = alloc_aligned(bytes);
    }

    if (v24_x86_workspace == NULL ||
        PQCP_MLDSA_NATIVE_MLDSA44_X86_64_v24_lifecycle_workspace_set(v24_x86_workspace, bytes) != 0) {
      die("failed to setup v24 x86_64 lifecycle workspace");
    }
  }

  if (PQCP_MLDSA_NATIVE_MLDSA44_C_v24_lifecycle_workspace_bytes &&
      PQCP_MLDSA_NATIVE_MLDSA44_C_v24_lifecycle_workspace_set) {
    size_t bytes = PQCP_MLDSA_NATIVE_MLDSA44_C_v24_lifecycle_workspace_bytes();

    if (v24_c_workspace == NULL) {
      v24_c_workspace = alloc_aligned(bytes);
    }

    if (v24_c_workspace == NULL ||
        PQCP_MLDSA_NATIVE_MLDSA44_C_v24_lifecycle_workspace_set(v24_c_workspace, bytes) != 0) {
      die("failed to setup v24 C lifecycle workspace");
    }
  }
}
...
static void run_sig_keypair(size_t iterations) {
  ...
  setup_v24_workspace_if_available();
